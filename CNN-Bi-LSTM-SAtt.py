...
class CNN_BiLSTM_SAtt(nn.Module):
    def __init__(self, input_dim, cnn_channels, lstm_hidden, num_layers, num_heads, output_dim, dropout=0.5):
        super(CNN_BiLSTM_SAtt, self).__init__()
        ...
        self.self_attn = nn.MultiheadAttention(
            embed_dim=lstm_hidden * 2,  # BiLSTM输出维度
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True)
    ...
    def forward(self, x):
        ...
        cnn_out = self.cnn(x)
        cnn_out = cnn_out.permute(0, 2, 1)  # (batch, new_seq_len, cnn_channels)

        # BiLSTM处理
        lstm_out, _ = self.lstm(cnn_out)  # (batch, new_seq_len, lstm_hidden*2)
